...
<div class="container">
    ...
    <button class="btn btn-primary" data-toggle="modal" data-target="#createAdminModal">Criar Novo Admin</button>
        ...
        <tbody>
            @foreach($admins as $admin)
                <tr>
                    ...
                    <td>{{ $admin->email }}</td>
                    <td>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#editAdminModal{{ $admin->id }}">Editar</button>
                        <form action="{{ route('useradmin.destroy', $admin->id) }}" method="POST" style="display:inline-block;">
                            @csrf
                            @method('DELETE')
                ...
                <div class="modal fade" id="editAdminModal{{ $admin->id }}" tabindex="-1" role="dialog" aria-labelledby="editAdminModalLabel{{ $admin->id }}" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action="{{ route('useradmin.update', $admin->id) }}" method="POST">
                                @csrf
                                @method('PUT')
